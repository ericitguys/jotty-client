...
mod tests {
    ...
    // Branding mirror (App.tsx) sets the window title from JS via
    // getCurrentWindow().setTitle. Tauri's ACL gates that call: `core:default`
    // grants only the read-only window getters (see tauri's
    // permissions/window/autogenerated/reference.md), so WITHOUT an explicit
    // grant the call is denied and the silent catch in App.tsx leaves the
    // titlebar on the conf default — reported 2026-09-26 as "branding changed
    // everything except the top bar". This fence pins the grant.
    #[test]
    fn capabilities_grant_window_set_title_for_branding_mirror() {
        ...
        let v: serde_json::Value = serde_json::from_str(&text).expect("valid capabilities JSON");
        let perms = v["permissions"].as_array().expect("permissions array");
        assert!(
            perms.iter().any(|p| p.as_str() == Some("core:window:allow-set-title")),
            "core:window:allow-set-title missing from capabilities/default.json: {perms:?}"
        );
    }
